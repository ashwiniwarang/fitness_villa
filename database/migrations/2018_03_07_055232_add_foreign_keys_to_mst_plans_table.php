...
class AddForeignKeysToMstPlansTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('mst_plans', function (Blueprint $table) {
            ...
            $table->foreign('created_by', 'FK_mst_plans_mst_users_1')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
            $table->foreign('updated_by', 'FK_mst_plans_mst_users_2')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
        });
    }
