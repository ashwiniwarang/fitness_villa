...
class AddForeignKeysToTrnExpensesTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('trn_expenses', function (Blueprint $table) {
            ...
            $table->foreign('created_by', 'FK_trn_expenses_mst_users_2')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
            $table->foreign('updated_by', 'FK_trn_expenses_mst_users_3')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
        });
    }
