...
class AddForeignKeysToTrnEnquiryFollowupsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('trn_enquiry_followups', function (Blueprint $table) {
            ...
            $table->foreign('created_by', 'FK_trn_enquiry_followups_mst_staff_2')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
            $table->foreign('updated_by', 'FK_trn_enquiry_followups_mst_staff_3')->references('id')->on('users')->onUpdate('RESTRICT')->onDelete('RESTRICT');
        });
    }
